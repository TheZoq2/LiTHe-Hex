...
float read_leg_float_from_file(uint8_t id, char* filename)
{
	...
	if(in_file == NULL)
	{
		printf("Failed to open servo state file %s, assuming done rotating\n", filename);
		return result;
	}

	char buffer[BUFFER_SIZE];

	uint8_t target_index = id;
	...
	while(done == false && current_index <= read_amount)
	{
...
{
	float result = read_leg_float_from_file(id, "/tmp/hexsim/servo_targets");
	return result;
}
