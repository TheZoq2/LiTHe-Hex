...
def main():
    while 1:
        ...
        offset = (CORRIDOR_WIDTH * (sensor_data[2] + SENSOR_OFFSET)/(sensor_data[1] + (2 * SENSOR_OFFSET) + sensor_data[2])) - (CORRIDOR_WIDTH/2)

        command_y = (sensor_data[1] - sensor_data[2]) * MOVEMENT_SCALEDOWN

        if abs(offset) > 0.1:
            goal_angle = -sensor_data[0] - math.pi * offset * ANGLE_SCALEDOWN
        else:
