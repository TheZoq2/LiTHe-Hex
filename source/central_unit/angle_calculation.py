...
def get_left_angle(sensor_data):
    return math.atan(math.fabs(sensor_data.ir_front_left - sensor_data.ir_back_left)/DISTANCE_BETWEEN_SENSORS) * (180/math.pi)

def get_average_angle(sensor_data):
    return ((get_left_angle(sensor_data) + get_right_angle(sensor_data))/2) * (180/math.pi)
